...
结论：确实存在 Photara 明显更好的照片，也存在已有重建更好的照片。不能把“偏离已有重建”直接当成错误，也不能把 739/739 注册或更低的自身重投影 RMS 当成全面胜出的证据。

## 对象与方法

- 已有模型：`D:/BaiduNetdiskDownload/室内iphone/sparse/0`，COLMAP 格式，738 张相机，222484 个点；生成算法未知，不是真值。
- Photara：`iphone_quality.asfm`，27000 特征配置，739 张相机，643244 个点；新匹配测试同时加入 `iphone_verified_sfm_diagnostics.csv` 的 6000 特征配置。
...
| 观测库 | 检查数 | 已有模型预测中位误差 / px | Photara 27000 预测中位误差 / px |
|---|---:|---:|---:|
| 已有轨迹 | 276644 | 1.227 | 1.547 |
| Photara 轨迹 | 281594 | 1.051 | 0.911 |

总体上各自在自己的轨迹库更好，所以不能用这两个整体平均数简单宣布获胜者。但个别照片有双向一致证据：DCIM5783.jpg 在已有轨迹库上为 5.71 / 0.91 px，在 Photara 轨迹库上为 99.56 / 0.56 px（已有 / Photara），都支持 Photara。
...
| 目标照片 | 对数 / 匹配数 | 已有模型 | Photara 6000 | Photara 27000 | 证据解释 |
|---|---:|---:|---:|---:|---|
| DCIM5783.jpg | 5 / 4225 | 4.58–23.22 | 0.17–0.48 | 0.16–0.34 | 多邻居一致支持 Photara |
| DCIM5833.jpg | 5 / 7397 | 3.77–8.13 | 0.19–0.44 | 0.19–0.23 | 多邻居一致支持 Photara |
...
具体而言，DCIM6228 与 DCIM6226 的距离为已有 22.14 / Photara 27000 2.14 px，但与 DCIM6172 为已有 1.72 / Photara 14.35 px。这提示局部变焦组内部可以自洽，但跨区域约束仍不正确；还不能仅凭这项检查确定单张相机或整组的唯一故障来源。

DCIM5012 的 27000 配置比 6000 更差也说明：增加特征、点数以及降低整体 RMS，不能保证每张相机的位置更正确。

## 对之前“质量门槛”的修正解释

原来的参考相机 Sim(3) 差异只能用作**差异报警**。其中最大异常 DCIM5783 的新匹配证据反而强烈支持 Photara，不应继续把它直接记为 Photara 的质量失败。应优先修复同时被独立像素证据判差的照片，例如 DCIM5012 和部分跨组连接。
