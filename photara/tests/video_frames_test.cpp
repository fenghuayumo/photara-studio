...
photara::io::RgbImage make_image(
...
photara::io::RgbImage make_checkerboard(
...
int main() {
    expect(photara::io::is_video_path("clip.mp4"), "mp4 is video");
    expect(photara::io::is_video_path("CLIP.MOV"), "MOV is video");
    expect(photara::io::is_video_path("dual.insv"), "insv is video");
    expect(!photara::io::is_video_path("photo.jpg"), "jpg is not video");
    expect(!photara::io::is_video_path("images"), "folder is not video");

    const auto frames = photara::io::default_video_frames_dir("D:/scan/clip.mp4");
    expect(
        ...
        "default frames dir is <stem>/images");
    {
        ...
        const auto chinese = photara::io::default_video_frames_dir(video);
        expect(
    ...
    const double sharp_score = photara::io::laplacian_sharpness(sharp);
    const double blur_score = photara::io::laplacian_sharpness(blur);
    expect(sharp_score > blur_score * 5.0, "checkerboard is sharper than flat");
    expect(sharp_score > 1.0, "checkerboard score is positive");

    const auto dir = std::filesystem::temp_directory_path() / "photara_video_frames_test";
    ...
    const int kept = photara::io::select_sharpest_frames(cand, out, 2, 0);
    ...
    const double kept0 = photara::io::laplacian_sharpness(out / "00000.png");
    const double kept1 = photara::io::laplacian_sharpness(out / "00001.png");
...

    {
        ...
        photara::io::save_rgb_png(sharp, occupied / "photo.png");
        photara::io::VideoExtractOptions occupied_job;
        occupied_job.video = dummy;
        occupied_job.output_dir = occupied;
        occupied_job.sharp_window = 1;
        bool threw = false;
        try {
            photara::io::extract_video_frames(occupied_job);
        ...
        expect(
            !photara::io::has_video_extract_manifest(occupied),
            "occupied folder gets no extract manifest");
        expect(
            !photara::io::has_matching_video_extract(occupied_job),
            "occupied folder is not a matching extract");
    }

    if (photara::io::ffmpeg_available()) {
        expect(
            !photara::io::locate_ffmpeg().empty(),
        ...
        if (std::system(synth.c_str()) == 0 &&
            ...
            try {
                ...
                const auto again = photara::io::extract_video_frames(options);
