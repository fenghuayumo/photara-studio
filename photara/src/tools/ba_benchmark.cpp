...
using photara::ba::Index;
using photara::ba::LinearizationOutput;
using photara::ba::PinholeIntrinsics;
using photara::ba::Point3;
using photara::ba::Pose;
using photara::ba::Problem;
...
BenchmarkConfig parse_arguments(const int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (argument == "--help") {
            std::cout
                << "Usage: photara_ba_benchmark [options]\n"
...
int main(int argc, char** argv) {
    try {
        ...
        if (config.gpu_solver_iterations > 0) {
            ...
            photara::ba::CudaOptimizer optimizer(optimizer_options);
        ...
        const auto cpu_stats = photara::ba::linearize_cpu(problem, cpu_output);
        ...
        photara::ba::CudaLinearizer cuda;
        cuda.upload(problem);
        cuda.evaluate();
        ...
        std::cout << "CUDA device: " << photara::ba::CudaLinearizer::device_name() << '\n'
