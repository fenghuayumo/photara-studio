...
[[nodiscard]] std::filesystem::path default_model_dir() {
    return std::filesystem::path(PHOTARA_INTRINSIC_MODELS_DIR_PATH);
}
...
public:
    explicit OnnxDelighter(const std::filesystem::path& model_dir, const bool use_cuda) {
        for (int i = 0; i < 4; ++i) {
            ...
            cfg.env_name = "Photara.Delighter";
...
void delight_texture_views(
    ...
    throw std::runtime_error(
        ...
        "-DPHOTARA_ENABLE_ONNX=ON and place stage_0..3.onnx under "
    ...
    if (!delighter_available(options)) {
        throw std::runtime_error(
            ...
            "PHOTARA_INTRINSIC_MODELS_DIR or TextureOptions::delight_model_dir");
    }
