...
[[nodiscard]] GaussianModel load_sog(const std::filesystem::path& path) {
    ...
    for (std::size_t index = 0; index < count; ++index) {
        ...
        // SOG's coordinate system is RUB; Photara uses RDF.
...
void pack_spz_quaternion(
    ...
    // SPZ's GaussianCloud uses xyzw; convert Photara's wxyz and RDF axes.
    std::array<float, 4> q{
...
[[nodiscard]] std::array<float, 4> unpack_spz_quaternion(const std::uint8_t input[4]) {
    ...
    // Convert SPZ xyzw/RUB back to Photara wxyz/RDF.
...
[[nodiscard]] GaussianModel load_spz(const std::filesystem::path& path) {
    ...
    if (magic == 0x5053474eU) {
        ...
        if (degree > 3)
            throw std::runtime_error("SPZ SH degree exceeds Photara support");
        if ((flags & 0x2U) != 0)
            throw std::runtime_error(
...
[[nodiscard]] Bytes save_sog(const GaussianModel& model) {
    ...
    meta << "{\"version\":2,\"asset\":{\"generator\":\"Photara\"},"
