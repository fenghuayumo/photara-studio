...
uint shape_at(uint axis)
{
    if (axis == 0)
    {
        return max(pc.shape0, 1u);
    }
    if (axis == 1)
    {
        return max(pc.shape1, 1u);
    }
    if (axis == 2)
    {
        return max(pc.shape2, 1u);
    }
    return max(pc.shape3, 1u);
}
...
void main(uint3 dtid : SV_DispatchThreadID)
{
    ...
    const uint ndim = max(pc.ndim, 1u);
    ...
    // Row-major coordinates, written as (slot, ndim) int64 values.
    uint coords[4];
