...
constexpr Entry k_entries[] = {
    ...
    {"Exit", "退出", "終了", "종료"},
    {"Selection", "选择", "選択", "선택"},
    {"Select All", "全选", "すべて選択", "모두 선택"},
    {"Deselect All", "取消全部选择", "選択をすべて解除", "선택 모두 해제"},
    {"Invert Selection", "反选", "選択を反転", "선택 반전"},
    {"Delete Selected", "删除选中", "選択を削除", "선택 삭제"},
    {"Select by centre", "按中心选择", "中心で選択", "중심으로 선택"},
    {"Select by ring", "按高斯环选择", "リングで選択", "링으로 선택"},
    {"A Gaussian counts only when its centre is inside the gesture.",
     "只有中心落在手势里的高斯会被选中。",
     "中心がジェスチャーの中にあるガウシアンだけを選びます。",
     "중심이 제스처 안에 있는 가우시안만 선택됩니다."},
    {"A Gaussian counts when its ring crosses the gesture. Wide splats are easier to grab, using the same ellipse as Rings view.",
     "高斯环碰到手势就选中。大块溅射更好抓，椭圆和 Rings 视图画的是同一套。",
     "リングがジェスチャーに掛かれば選択します。広いスプラットを掴みやすく、Rings 表示と同じ楕円です。",
     "링이 제스처에 걸리면 선택합니다. 넓은 스플랫을 잡기 쉽고, Rings 보기와 같은 타원입니다."},
    {"By ring", "按环", "リング", "링"},
    ...
    {"About Photara Studio", "关于 Photara Studio", "Photara Studio について", "Photara Studio 정보"},
    {"Photogrammetry and 3D Gaussian splatting",
     "摄影测量与 3D 高斯溅射",
     "フォトグラメトリと 3D ガウシアン・スプラッティング",
     "포토그래메트리와 3D 가우시안 스플래팅"},
    {"Version", "版本", "バージョン", "버전"},
    {"Pipeline", "流程", "パイプライン", "파이프라인"},
    {"SfM / MVS / 3DGS / mesh / texture",
     "SfM / MVS / 3DGS / 网格 / 贴图",
     "SfM / MVS / 3DGS / メッシュ / テクスチャ",
     "SfM / MVS / 3DGS / 메시 / 텍스처"},
    {"Backends", "后端", "バックエンド", "백엔드"},
    {"Developer", "开发者", "開発元", "개발사"},
    {"Copyright", "版权", "著作権", "저작권"},
    ...
    {"Select a capture video. Align Photos extracts frames, generates masks, then aligns cameras.",
     "选择采集视频。对齐照片会先抽帧，再生成蒙版，然后对齐相机。",
     "撮影動画を選択。フレーム抽出、マスク生成、カメラアライメントの順です。",
     "촬영 동영상을 선택하세요. 프레임 추출, 마스크 생성, 카메라 정렬 순서입니다."},
    ...
    {"Extract a surface with the MVS mesh pipeline after alignment.\n"
     ...
     "对齐后用 MVS 网格流程提取表面。\n在网格面板中选择从 MVS 或从高斯生成。",
     "アライメント後に MVS メッシュパイプラインで表面を抽出します。\nメッシュパネルで「MVS から」または「ガウシアンから」を選んでください。",
     "정렬 후 MVS 메시 파이프라인으로 표면을 추출합니다.\n메시 패널에서 MVS에서 또는 가우시안에서를 선택하세요."},
    ...
    {"3D scene view", "3D 场景视图", "3D シーンビュー", "3D 장면 뷰"},
    {"2D image QA — features, GT vs 3DGS, error map",
     "2D 图像质检 — 特征、真值对比 3DGS、误差图",
     "2D 画像 QA — 特徴、GT と 3DGS、誤差マップ",
     "2D 이미지 QA — 특징, GT vs 3DGS, 오차 맵"},
    {"keypoints", "特征点", "キーポイント", "키포인트"},
    {"Keypoints", "特征点", "キーポイント", "키포인트"},
    {"inliers", "内点", "インライア", "인라이어"},
    {"matches", "组匹配", "マッチ", "매칭"},
    {"Verified pairs", "已验证配对", "検証済みペア", "검증된 페어"},
    {"Show unmatched keypoints", "显示未匹配特征点", "未マッチのキーポイントを表示", "미매칭 키포인트 표시"},
    {"brighter line = higher score", "线越亮分数越高", "線が明るいほどスコアが高い", "선이 밝을수록 점수가 높음"},
    {"line = verified correspondence", "线 = 已验证对应", "線 = 検証済み対応", "선 = 검증된 대응"},
    {"score", "分数", "スコア", "점수"},
    {"Loading pair…", "正在加载匹配图…", "ペアを読み込み中…", "페어 불러오는 중…"},
    {"Reading the matched image", "正在读取匹配图像", "照合画像を読み込み中", "매칭 이미지를 읽는 중"},
    ...
    {"Gaussian Splats", "Gaussian Splats", "Gaussian Splats", "Gaussian Splats"},
    {"Undo", "撤销", "元に戻す", "실행 취소"},
    {"Redo", "重做", "やり直す", "다시 실행"},
    {"Navigate", "环绕", "オービット", "궤도"},
    {"Pick", "点选", "ピック", "선택"},
    {"Circle select", "圆形选择", "円選択", "원형 선택"},
    {"Polygon select", "多边形选择", "多角形選択", "다각형 선택"},
    {"Brush select", "画笔选择", "ブラシ選択", "브러시 선택"},
    {"Front surface", "只选最前一层", "手前の面だけ", "앞면만 선택"},
    {"All depths", "穿过所有层", "すべての奥行き", "모든 깊이"},
    {"selected", "已选", "選択", "선택"},
    {"Undo the last selection.", "撤销上一次选择。", "直前の選択を元に戻します。", "마지막 선택을 실행 취소합니다."},
    {"Redo the last selection.", "重做上一次选择。", "直前の選択をやり直します。", "마지막 선택을 다시 실행합니다."},
    {"Left button orbits. A click does not select until a selection tool is on.",
     "左键环绕场景。未打开选择工具时，点击不会选中高斯。",
     "左ボタンでオービットします。選択ツールを開くまでクリックでは選ばれません。",
     "왼쪽 버튼으로 장면을 돌립니다. 선택 도구를 켜기 전에는 클릭해도 선택되지 않습니다."},
    {"Click one Gaussian, or drag a rectangle. Shift adds and Alt removes.",
     "单击选中一个高斯，拖拽则框选。Shift 加选，Alt 减选。",
     "クリックで一つ、ドラッグで矩形。Shift で追加、Alt で除外。",
     "클릭으로 하나, 드래그로 상자 선택. Shift는 추가, Alt는 제거입니다."},
    {"Drag out a circle. A click without dragging leaves the selection unchanged.",
     "按住拖出一个圆。几乎没移动的点击不会改变选择。",
     "円をドラッグします。ほとんど動かないクリックは選択を変えません。",
     "원을 드래그합니다. 거의 움직이지 않은 클릭은 선택을 바꾸지 않습니다."},
    {"Click to add corners. Close on the first point or press Enter. Backspace removes the last corner.",
     "单击添加顶点。靠近起点或按 Enter 闭合。Backspace 删除最后一个点。",
     "クリックで頂点を追加。始点付近か Enter で閉じ、Backspace で最後の点を削除。",
     "클릭하여 꼭짓점을 추가합니다. 시작점 근처 또는 Enter로 닫고, Backspace로 마지막 점을 지웁니다."},
    {"Paint over Gaussians. [ ] or Alt+wheel changes the brush size.",
     "按住涂抹高斯。[ ] 或 Alt+滚轮调整笔刷大小。",
     "ガウシアンを塗ります。[ ] または Alt+ホイールで太さを変えます。",
     "가우시안 위를 칠합니다. [ ] 또는 Alt+휠로 브러시 크기를 바꿉니다."},
    {"The next gesture keeps only the nearest Gaussian in each small screen cell.",
     "之后的选择在每个小格里只保留离相机最近的高斯。",
     "次の選択は画面の小区間ごとに、最も近いガウシアンだけを残します。",
     "다음 선택은 화면의 작은 칸마다 가장 가까운 가우시안만 남깁니다."},
    {"The next gesture selects every layer it covers, including Gaussians hidden behind the front one.",
     "之后的选择会包含范围内每一层，包括被挡住的高斯。",
     "次の選択は手前に隠れているものも含め、範囲内のすべての層を選びます。",
     "다음 선택은 앞에 가려진 것을 포함해 범위 안의 모든 층을 고릅니다."},
    {"Click or drag a box", "单击或框选", "クリックまたは矩形", "클릭 또는 상자"},
    {"Drag a circle", "拖拽画圆", "ドラッグで円", "드래그하여 원"},
    {"Click points, Enter closes", "单击加点，Enter 闭合", "クリックで頂点、Enter で閉じる", "클릭으로 점, Enter로 닫기"},
    {"Paint across Gaussians", "按住涂选", "ドラッグで塗る", "드래그하여 칠하기"},
    {"Shift add  |  Alt remove  |  RMB orbit  |  Esc steps back",
     "Shift 加选  |  Alt 减选  |  右键环绕  |  Esc 逐步返回",
     "Shift追加  |  Alt除外  |  右オービット  |  Escで段階的に戻る",
     "Shift 추가  |  Alt 제거  |  오른쪽 궤도  |  Esc로 단계적으로 복귀"},
    {"Esc clears the selection", "Esc 清空选择", "Escで選択解除", "Esc로 선택 해제"},
    {"Delete removes the selection", "Delete 删除选中", "Deleteで選択を削除", "Delete로 선택 삭제"},
    {"Ctrl+Z restores the deleted Gaussians",
     "Ctrl+Z 恢复刚删除的高斯",
     "Ctrl+Z で削除したガウシアンを戻す",
     "Ctrl+Z로 삭제한 가우시안을 복원"},
    {"gaussians removed", "个高斯已删除", "個のガウシアンを削除", "개 가우시안을 삭제함"},
    {"LMB orbit  |  MMB pan  |  RMB + WASD/QE fly  |  F frame  |  double-click focus",
     "左键环绕  |  中键平移  |  右键 + WASD/QE 飞行  |  F 框住场景  |  双击聚焦",
     "左オービット  |  中パン  |  右 + WASD/QE 飛行  |  F で全体表示  |  ダブルクリックでフォーカス",
     "왼쪽 궤도  |  가운데 이동  |  오른쪽 + WASD/QE 비행  |  F 전체 보기  |  더블클릭 초점"},
    ...
    {"Align Photos extracts frames, generates masks, then aligns cameras.",
     "对齐照片会先抽帧，再生成蒙版，然后对齐相机。",
     "フレーム抽出、マスク生成、カメラアライメントの順です。",
     "프레임 추출, 마스크 생성, 카메라 정렬 순서입니다."},
    {"Align Photos generates masks, then aligns cameras.",
     "对齐照片会先生成蒙版，再对齐相机。",
     "マスクを生成してからカメラをアライメントします。",
     "마스크를 생성한 뒤 카메라를 정렬합니다."},
    ...
    {"Include a surface mesh in the reconstruction.\n"
     ...
     "在重建中包含表面网格。\n在下方选择从高斯生成或从 MVS。",
     "再構成に表面メッシュを含めます。\n下で「ガウシアンから」か「MVS から」を選択。",
     "재구성에 표면 메시를 포함합니다.\n아래에서 가우시안에서 또는 MVS에서를 선택하세요."},
    {"Method", "方法", "方法", "방법"},
    ...
    {"Last reconstruction step. Unwrap the mesh, project calibrated "
     ...
     "重建最后一步。展开网格 UV，用 photara_drender 投影已标定照片，可选优化图集。",
     "再構成の最終段。メッシュを展開し photara_drender で校正写真を投影、任意でアトラスを洗練。",
     "재구성 마지막 단계. 메시를 펼치고 photara_drender로 보정 사진을 투영한 뒤 선택적으로 아틀라스를 다듬습니다."},
    {"This build was compiled without texture baking (Vulkan + photara_drender).",
     "此构建未包含纹理烘焙（Vulkan + photara_drender）。",
     "このビルドはテクスチャベイクなし（Vulkan + photara_drender）。",
     "이 빌드는 텍스처 베이크 없이 컴파일되었습니다(Vulkan + photara_drender)."},
    {"Quality", "质量", "品質", "품질"},
    {"Fast", "快速", "高速", "빠르게"},
    {"Standard", "标准", "標準", "표준"},
    {"High", "高", "高", "높음"},
    ...
    {"Photometric + seam optimization in photara_drender after the "
     ...
     "投影烘焙后在 photara_drender 中做光度与接缝优化。关闭更快；开启可清理视角接缝。",
     "投影ベイク後に photara_drender で測光とシーム最適化。オフは速い、オンは視点シームを整えます。",
     "투영 베이크 후 photara_drender에서 측광·시임 최적화. 끄면 빠르고, 켜면 시점 시임을 정리합니다."},
    {"Status", "状态", "状態", "상태"},
    ...
    {"Unwrap UVs and project the calibrated photos onto the current mesh.",
     "展开 UV，并把已标定照片投影到当前网格上。",
     "UV を展開し、校正済み写真を現在のメッシュへ投影します。",
     "UV를 펼치고 보정된 사진을 현재 메시에 투영합니다."},
    {"Unwrap UVs again and reproject the calibrated photos onto the current mesh.",
     "重新展开 UV，并把已标定照片再次投影到当前网格上。",
     "UV を展開し直し、校正済み写真を現在のメッシュへ再投影します。",
     "UV를 다시 펼치고 보정된 사진을 현재 메시에 재투영합니다."},
    {"Train appearance-only 3DGS from the aligned cameras. The mesh stays on the MVS path.",
     "从已对齐相机训练仅外观的 3DGS。网格仍走 MVS 路径。",
     "アライメント済みカメラから見た目のみの 3DGS を学習。メッシュは MVS 経路のままです。",
     "정렬된 카메라로 외형만 학습하는 3DGS를 실행합니다. 메시는 MVS 경로를 유지합니다."},
    {"Extract a surface from the Gaussians.\n"
     "Trains with depth and normal losses, then builds the selected surface.",
     "从高斯提取表面。\n先用深度与法向损失训练，再按所选方法生成表面。",
     "ガウシアンから表面を抽出。\n深度・法線ロスで学習してから、選択した方法で表面を作ります。",
     "가우시안에서 표면을 추출합니다.\n깊이·법선 손실로 학습한 뒤 선택한 방법으로 표면을 만듭니다."},
    {"Extract a surface from the trained 3DGS model.\n"
     "Uses the saved Gaussians. Training is not started again.",
     "从已训练的 3DGS 模型提取表面。\n使用已保存的高斯，不会重新开始训练。",
     "学習済み 3DGS モデルから表面を抽出。\n保存済みガウシアンを使い、学習はやり直しません。",
     "학습된 3DGS 모델에서 표면을 추출합니다.\n저장된 가우시안을 사용하며 학습을 다시 시작하지 않습니다."},
    {"Build a mesh from the MVS dense cloud.\n"
     "PatchMatch stereo from the aligned cameras, then fuse triangles.",
     "从 MVS 稠密点云生成网格。\n对已对齐相机做 PatchMatch，再融合三角形。",
    ...
    {"Frames", "抽帧", "フレーム抽出", "프레임 추출"},
    {"Masks", "蒙版", "マスク", "마스크"},
    {"Extracting frames", "正在抽帧", "フレーム抽出中", "프레임 추출 중"},
    {"Generating masks", "正在生成蒙版", "マスク生成中", "마스크 생성 중"},
    {"Sharp frames are chosen before masks and alignment",
     "先选出清晰帧，再生成蒙版并对齐相机。",
     "鮮明なフレームを選んでからマスクとアライメントを行います。",
     "선명한 프레임을 고른 뒤 마스크와 정렬을 진행합니다."},
    {"Each frame gets a foreground mask, then alignment uses it",
     "每一帧先得到前景蒙版，对齐时会用上它。",
     "各フレームの前景マスクを作ってからアライメントに使います。",
     "각 프레임의 전경 마스크를 만든 뒤 정렬에 사용합니다."},
    {"Features", "特征", "特徴", "특징"},
    {"Matching", "匹配", "マッチング", "매칭"},
    {"Tracks", "轨迹", "トラック", "트랙"},
    {"Poses", "位姿", "姿勢", "포즈"},
    {"Extracting features in 2D", "正在 2D 中提取特征", "2D で特徴抽出中", "2D에서 특징 추출 중"},
    {"Switch to 2D to watch keypoints appear on each photo",
     "切换到 2D 查看每张照片上的特征点",
     "2D に切り替えて各写真のキーポイントを確認",
     "2D로 전환해 각 사진의 키포인트를 확인하세요"},
    {"Matching images in 2D", "正在 2D 中匹配图像", "2D で画像マッチング中", "2D에서 이미지 매칭 중"},
    {"Switch to 2D to watch correspondence lines between pairs",
     "切换到 2D 查看图像对之间的对应线",
     "2D に切り替えてペア間の対応線を確認",
     "2D로 전환해 이미지 쌍의 대응선을 확인하세요"},
    ...
    {"Video selected. Align Photos extracts frames, generates masks, then aligns cameras",
     "已选择视频。对齐照片会先抽帧，再生成蒙版，然后对齐相机。",
     "動画を選択しました。フレーム抽出、マスク生成、カメラアライメントの順です。",
     "동영상을 선택했습니다. 프레임 추출, 마스크 생성, 카메라 정렬 순서입니다."},
