...
#pragma unroll
    ...
    for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) {
        ...
        for (int j = 0; j < min(BLOCK_SIZE, toDo); j++) {
            ...
            float dL_drefine_weight_local                = 0.0f;
            if (valid) {
                T                           = T / (1.f - alpha);
...
#pragma unroll
                ...
                const float dL_dG_render = con_o.w * dL_dopa_render;
                ...
                // dL_ddelx_render / dL_ddely_render keep only the render-branch
                // contribution (RGB + alpha/background), matching gsplatrast/fastergs.
                // GGGS geometry-specific paths affect opacity/finalT as well as the
                // explicit dL_dt * ray_plane term, so the densification signal must
                // exclude all of them.
                ...
                // Brush / gsplat-style refine signal used for densification:
                //   sum_pixels ||(dL/d_mean2d_pixel_render) * (W, H)|| / max(final_alpha, 1e-5)
                // Use only the render branch here. Geometry gradients (normal / mdepth)
                // otherwise leak in through both dL_dt * ray_plane and dL_dopa, which
                // makes GGGS refine_weight much larger than gsplatrast/fastergs and
                // causes over-densification.
                if (refine_weight != nullptr) {
                    const float gx_pix  = dL_ddelx_render * static_cast<float>(W);
                    const float gy_pix  = dL_ddely_render * static_cast<float>(H);
                    const float final_a = fmaxf(w_final, 1e-5f);
                    dL_drefine_weight_local = sqrtf(gx_pix * gx_pix + gy_pix * gy_pix) / final_a;
                }
            }
            ...
            if (refine_weight != nullptr)
                warpSum(dL_drefine_weight_local, warp);
            if constexpr (GEOMETRY) {
...
#pragma unroll
                ...
                if (refine_weight != nullptr)
                    atomicAdd(&refine_weight[global_id], dL_drefine_weight_local);
                if constexpr (GEOMETRY) {
