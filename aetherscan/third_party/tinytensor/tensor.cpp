...
#include <iomanip>
#include <numeric>
// #include <print>  // C++23 - replaced with std::cout
...
namespace tinytensor {
    ...
    void Tensor::relabel_allocation_for_profiler() {
        ...
        try {
            tinytensor::VramProfiler::instance().relabelAllocation(data_, state_->name);
        } catch (...) {
...
#undef CONVERT_DTYPE_CUDA
        ...
        if (state_ && state_->has_deferred_expr) {
            ...
            Tensor deferred = make_deferred_expr_tensor(
                ...
                [source = std::move(source), deferred_shape]() mutable {
                    ...
                    return tinytensor::broadcast_to(materialized, deferred_shape);
                },
                std::move(deferred_inputs));
            deferred.set_stream(source.stream());
            return deferred;
        }
        return tinytensor::broadcast_to(*this, target_shape);
    }
    ...
    void Tensor::print_formatted(const std::string& name, size_t max_per_dim) const {
        std::cout << std::format("\n=== {} ===", name.empty() ? "Tensor" : name) << std::endl;
        ...
        if (shape_.rank() == 1) {
            ...
            std::cout << "  [Higher dimensional tensor - showing first slice]" << std::endl;
    ...
    void Tensor::print_1d(size_t max_elem) const {
        ...
        for (size_t i = 0; i < values.size(); ++i) {
            if (i > 0)
                std::cout << ", ";
            std::cout << std::format("{:8.4f}", values[i]);
        }

        if (numel() > max_elem) {
            std::cout << std::format(", ... ({} more)", numel() - max_elem);
        }
        std::cout << "]" << std::endl;
    }
    ...
    void Tensor::print_2d(size_t max_per_dim) const {
        ...
        for (size_t i = 0; i < rows; ++i) {
            ...
            for (size_t j = 0; j < cols; ++j) {
                ...
                std::cout << std::format("{:8.4f}", values[idx]);
            }
            if (shape_[1] > cols) {
                std::cout << std::format(", ... ({} more)", shape_[1] - cols);
            }
            std::cout << "]";

            if (i == rows - 1 && shape_[0] > rows) {
                std::cout << std::format("  ... ({} more rows)", shape_[0] - rows);
            }
            std::cout << std::endl;
        }
    }
    ...
    void Tensor::dump_diagnostic(const std::string& filename) const {
        std::ofstream file;
        if (!tinytensor::open_file_for_write(tinytensor::utf8_to_path(filename), file)) {
