...
namespace tinytensor::tensor_ops {
    ...
    YISON_CORE_API void launch_binary_op_generic(const InT* a, const InT* b, OutT* c, size_t n,
                                               Op op, cudaStream_t stream = nullptr);

    template <typename T, typename OutT, typename Op>
    YISON_CORE_API void launch_unary_op_generic(const T* input, OutT* output, size_t n,
                                              Op op, cudaStream_t stream = nullptr);

    template <typename T, typename OutputT, typename Op>
    YISON_CORE_API void launch_scalar_op_generic(const T* data, T scalar, OutputT* result, size_t n,
                                               Op op, cudaStream_t stream = nullptr);
} // namespace tinytensor::tensor_ops
...
namespace tinytensor::tensor_ops {
    ...
    YISON_CORE_API void launch_clamp_scalar(float* data, float min_val, float max_val, size_t n, cudaStream_t stream);
    YISON_CORE_API void launch_clamp_fused(const float* src, float* dst, float min_val, float max_val, size_t n, cudaStream_t stream);
    YISON_CORE_API void launch_clamp_scalar_int(int* data, int min_val, int max_val, size_t n, cudaStream_t stream);

    YISON_CORE_API void launch_reduce_op(const void* input, void* output,
    ...
    YISON_CORE_API void launch_warp_reduce_full(const float* input, float* output, size_t n,
                                              ReduceOp op, cudaStream_t stream);

    YISON_CORE_API void launch_warp_segmented_reduce(const float* input, float* output,
                                                   size_t num_segments, size_t segment_size,
                                                   ReduceOp op, cudaStream_t stream);

    YISON_CORE_API void launch_warp_strided_reduce(const float* input, float* output,
                                                 size_t outer_size, size_t reduce_size, size_t inner_size,
                                                 ReduceOp op, cudaStream_t stream);

    YISON_CORE_API void launch_warp_multi_axis_reduce(const float* input, float* output,
                                                    size_t output_size, size_t reduce_count,
                                                    ReduceOp op, cudaStream_t stream);

    YISON_CORE_API bool should_use_warp_reduce(size_t n, size_t num_segments);

    // ============= Column Reduction (dim=0 for 2D matrices) =============
    // Faster than transpose+contiguous+reduce for column sums
    YISON_CORE_API void launch_column_reduce(const float* input, float* output,
                                           size_t M, size_t N, ReduceOp op, cudaStream_t stream);

    // ============= Direct Scalar Reductions (Fast Path) =============
    YISON_CORE_API float direct_sum_scalar(const float* data, size_t n, cudaStream_t stream);
    YISON_CORE_API float direct_mean_scalar(const float* data, size_t n, cudaStream_t stream);
    YISON_CORE_API float direct_max_scalar(const float* data, size_t n, cudaStream_t stream);
    YISON_CORE_API float direct_min_scalar(const float* data, size_t n, cudaStream_t stream);

    // ============= Load Operations =============
    YISON_CORE_API void launch_load_op(void* output, const size_t* shape, size_t rank,
    ...
    // ============= Broadcasting =============
    YISON_CORE_API void launch_broadcast(const float* src, float* dst,
                                       const size_t* src_shape, const size_t* dst_shape,
                                       size_t src_rank, size_t dst_rank,
                                       size_t dst_elements, cudaStream_t stream);

    YISON_CORE_API void launch_broadcast_bool(const unsigned char* src, unsigned char* dst,
                                            const size_t* src_shape, const size_t* dst_shape,
                                            size_t src_rank, size_t dst_rank,
                                            size_t dst_elements, cudaStream_t stream);

    YISON_CORE_API void launch_broadcast_strided(const float* src, float* dst,
                                               const size_t* src_shape, const size_t* src_strides,
                                               const size_t* dst_shape,
                                               size_t src_rank, size_t dst_rank,
                                               size_t dst_elements, cudaStream_t stream);

    YISON_CORE_API void launch_broadcast_strided_bool(const unsigned char* src, unsigned char* dst,
                                                    const size_t* src_shape, const size_t* src_strides,
                                                    const size_t* dst_shape,
                                                    size_t src_rank, size_t dst_rank,
                                                    size_t dst_elements, cudaStream_t stream);

    YISON_CORE_API void launch_pad(const float* src, float* dst,
                                 const size_t* src_shape, const size_t* src_strides,
...
namespace tinytensor::tensor_ops {
    ...
    YISON_CORE_API void launch_broadcast_binary(const T* a, const T* b, OutputT* c,
...
namespace tinytensor::tensor_ops {
    ...
    // ============= Matrix Operations =============
    YISON_CORE_API void launch_matmul(const float* a, const float* b, float* c,
                                    size_t m, size_t n, size_t k,
                                    cudaStream_t stream);

    YISON_CORE_API void launch_batch_matmul(const float* a, const float* b, float* c,
                                          size_t batch_size, size_t m, size_t n, size_t k,
                                          cudaStream_t stream);

    YISON_CORE_API void launch_transpose(const float* input, float* output,
                                       size_t rows, size_t cols,
                                       cudaStream_t stream);

    YISON_CORE_API void launch_dot_product(const float* a, const float* b, float* result,
                                         size_t n, cudaStream_t stream);

    // ============= Random Operations =============
    YISON_CORE_API void launch_uniform(float* data, size_t n, float low, float high,
                                     unsigned long long seed, cudaStream_t stream);

    YISON_CORE_API void launch_normal(float* data, size_t n, float mean, float std,
                                    unsigned long long seed, cudaStream_t stream);

    YISON_CORE_API void launch_bernoulli(float* data, size_t n, float p,
                                       unsigned long long seed, cudaStream_t stream);

    YISON_CORE_API void launch_randint(int* data, size_t n, int low, int high,
                                     unsigned long long seed, cudaStream_t stream);

    YISON_CORE_API void launch_multinomial(const float* weights, int64_t* samples,
                                         unsigned long n, unsigned long num_samples, bool replacement,
                                         unsigned long long seed, cudaStream_t stream);

    // ============= Matrix Creation Operations =============
    YISON_CORE_API void launch_eye(float* data, size_t m, size_t n, cudaStream_t stream);
    YISON_CORE_API void launch_diag(const float* diagonal, float* matrix, size_t n, cudaStream_t stream);
    YISON_CORE_API void launch_extract_diag(const float* matrix, float* diagonal, size_t n, cudaStream_t stream);

    YISON_CORE_API void launch_sgemm(const float* a, const float* b, float* c,
                                   size_t m, size_t n, size_t k, cudaStream_t stream);
    YISON_CORE_API void launch_sgemm_tn(const float* a, const float* b, float* c,
                                      size_t m, size_t n, size_t k, cudaStream_t stream);
    YISON_CORE_API void launch_sgemm_batched(const float* a, const float* b, float* c,
                                           size_t batch, size_t m, size_t n, size_t k,
                                           cudaStream_t stream);
    YISON_CORE_API void launch_sgemm_bias_relu(const float* a, const float* b, const float* bias, float* c,
                                             size_t m, size_t n, size_t k, cudaStream_t stream);

    // ============= Masking Operations =============
    YISON_CORE_API void launch_masked_select(const float* input, const unsigned char* mask,
                                           float* output, size_t n, size_t output_size, cudaStream_t stream);

    YISON_CORE_API void launch_masked_fill(float* data, const unsigned char* mask,
                                         float value, size_t n, cudaStream_t stream);
    YISON_CORE_API void launch_masked_fill(int32_t* data, const unsigned char* mask,
                                         int32_t value, size_t n, cudaStream_t stream);
    YISON_CORE_API void launch_masked_fill(int64_t* data, const unsigned char* mask,
                                         int64_t value, size_t n, cudaStream_t stream);
    YISON_CORE_API void launch_masked_fill(uint8_t* data, const unsigned char* mask,
                                         uint8_t value, size_t n, cudaStream_t stream);
    YISON_CORE_API void launch_masked_fill(__half* data, const unsigned char* mask,
                                         __half value, size_t n, cudaStream_t stream);

    YISON_CORE_API void launch_masked_scatter(float* data, const unsigned char* mask,
                                            const float* src, size_t n, size_t src_size, cudaStream_t stream);

    YISON_CORE_API void launch_where(const unsigned char* condition,
    ...
    YISON_CORE_API void launch_count_nonzero_bool(const unsigned char* data, size_t* count,
                                                size_t n, cudaStream_t stream);

    YISON_CORE_API void launch_count_nonzero_float(const float* data, size_t* count,
                                                 size_t n, cudaStream_t stream);

    // ============= Indexing Operations =============
    YISON_CORE_API void launch_index_select(const float* input, const int* indices, float* output,
                                          const size_t* shape, size_t rank, int dim,
                                          size_t index_size, int boundary_mode, cudaStream_t stream);

    YISON_CORE_API void launch_index_select(const int64_t* input, const int* indices, int64_t* output,
                                          const size_t* shape, size_t rank, int dim,
                                          size_t index_size, int boundary_mode, cudaStream_t stream);

    YISON_CORE_API void launch_index_select(const int32_t* input, const int* indices, int32_t* output,
                                          const size_t* shape, size_t rank, int dim,
                                          size_t index_size, int boundary_mode, cudaStream_t stream);

    YISON_CORE_API void launch_index_select(const uint8_t* input, const int* indices, uint8_t* output,
                                          const size_t* shape, size_t rank, int dim,
                                          size_t index_size, int boundary_mode, cudaStream_t stream);

    YISON_CORE_API void launch_gather(const float* input, const int* indices, float* output,
    ...
    YISON_CORE_API void launch_gather(const int64_t* input, const int* indices, int64_t* output,
                                    const size_t* input_shape, const size_t* index_shape,
                                    size_t rank, int dim, size_t total_elements,
                                    int boundary_mode, cudaStream_t stream);

    YISON_CORE_API void launch_take(const float* input, const int* indices, float* output,
    ...
    YISON_CORE_API void launch_zip_gather_2(const float* input1, const float* input2,
                                          const int* indices,
    ...
    YISON_CORE_API void launch_zip_gather_3(const float* input1, const float* input2, const float* input3,
                                          const int* indices,
    ...
    YISON_CORE_API void launch_index_put(float* data, const int* indices, const float* values,
                                       size_t data_size, size_t index_size, cudaStream_t stream);

    YISON_CORE_API size_t launch_nonzero(const float* data, int64_t* indices,
                                       size_t n, size_t output_size, cudaStream_t stream);

    YISON_CORE_API size_t launch_nonzero_bool(const unsigned char* data, int64_t* indices,
                                            size_t n, size_t output_size, cudaStream_t stream);

    // ============= Cumulative Sum Operation =============
    YISON_CORE_API void launch_cumsum(void* data, const size_t* shape, size_t rank,
                                    int dim, DataType dtype, cudaStream_t stream);

    // ============= Pairwise Distance Operations =============
    YISON_CORE_API void launch_cdist(const float* a, const float* b, float* out,
                                   size_t N, size_t M, size_t D, float p, cudaStream_t stream);

    // ============= Sorting Operations =============
    YISON_CORE_API void launch_sort_1d(float* values, int64_t* indices, size_t n,
                                     bool descending, cudaStream_t stream);

    YISON_CORE_API void launch_sort_2d(float* values, int64_t* indices,
                                     size_t outer_size, size_t dim_size, size_t inner_size,
                                     int dim, bool descending, cudaStream_t stream);

    // ============= Concatenation Operations =============
    YISON_CORE_API void launch_cat_last_dim(void* output, const std::vector<Tensor>& tensors, size_t num_rows,
                                          size_t row_size, size_t element_size, cudaStream_t stream);

    YISON_CORE_API void launch_cat_middle_dim(void* output, const std::vector<Tensor>& tensors, size_t outer_size, size_t inner_size,
                                            int resolved_dim, size_t element_size, cudaStream_t stream);

    YISON_CORE_API void launch_strided_copy(
        const void* input, void* output,
        const size_t* shape, const size_t* strides,
        size_t rank, size_t total_elements,
        DataType dtype, cudaStream_t stream = nullptr);

    YISON_CORE_API void launch_strided_copy_immediate(
        const void* input, void* output,
        const std::vector<size_t>& shape, const std::vector<size_t>& strides,
        size_t total_elements, DataType dtype, cudaStream_t stream = nullptr);

    YISON_CORE_API void launch_strided_upload(
    ...
    YISON_CORE_API void launch_strided_scatter(
        const void* input, void* output,
        const size_t* d_shape, const size_t* d_strides,
        size_t rank, size_t total_elements,
        DataType dtype, cudaStream_t stream = nullptr);

    YISON_CORE_API void launch_strided_scatter_immediate(
        const void* input, void* output,
        const std::vector<size_t>& shape, const std::vector<size_t>& strides,
        size_t total_elements, DataType dtype, cudaStream_t stream = nullptr);

    YISON_CORE_API void launch_strided_scatter_int32_to_float32(
        const void* input,
        void* output,
        const size_t* d_shape,
        const size_t* d_strides,
    ...
    YISON_CORE_API bool has_nan_or_inf_gpu(const float* data, size_t n, cudaStream_t stream = nullptr);

    // ============= Fused Affine Transform =============
    YISON_CORE_API void launch_fused_affine_transform(const float* input, float* output,
    ...
    YISON_CORE_API void launch_fused_pointwise_chain(const float* input, float* output,
                                                   size_t n, const FusedPointwiseOpChain& chain,
                                                   cudaStream_t stream = nullptr);

    YISON_CORE_API void launch_fused_transform_reduce(const float* input, float* output, size_t n,
                                                    const FusedPointwiseOpChain& chain,
                                                    ReduceOp reduce_op, cudaStream_t stream = nullptr);

    YISON_CORE_API void launch_fused_segmented_transform_reduce(
