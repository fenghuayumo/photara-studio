...
void test_projection_conventions() {
    ...
    expect(std::abs(camera.pixel_error_to_angular(1.0) -
                    (2.0 * k_pi / 1024.0) *
                        aetherscan::k_equirect_threshold_scale) < 1e-15,
...
void test_relative_pose_equirect() {
    ...
    const auto relative = aetherscan::sfm::estimate_relative_pose(
    ...
    const auto degenerate = aetherscan::sfm::estimate_relative_pose(
...
void test_absolute_pose_equirect() {
    ...
    const auto absolute = aetherscan::sfm::estimate_absolute_pose(
    ...
    const auto pinhole_pose = aetherscan::sfm::estimate_absolute_pose(
...
void test_triangulation_equirect() {
    ...
    const unsigned triangulated = aetherscan::sfm::triangulate_tracks(
        scene, false, aetherscan::sfm::TriangulationOptions{});
    ...
    const auto stats = aetherscan::sfm::filter_tracks(scene, 3.F, 0.5F);
...
void test_bundle_adjustment_equirect() {
    ...
    for (std::size_t image = 0; image < poses.size(); ++image) {
        for (std::size_t i = 0; i < points.size(); ++i) {
            const Vec2 pixel = camera.project(
                poses[image].transform_world_to_camera(points[i]));
            problem.observations.push_back(static_cast<aetherscan::ba::Index>(image),
                                           static_cast<aetherscan::ba::Index>(i),
                                           pixel.x(), pixel.y());
        }
    }
    problem.validate();

    const double exact_cost = aetherscan::ba::evaluate_cost(problem, 2.0);
    ...
    for (std::size_t image = 1; image < problem.poses.size(); ++image) {
        aetherscan::ba::Pose& pose = problem.poses[image];
    ...
    const double initial_cost = aetherscan::ba::evaluate_cost(problem, 2.0);
    expect(std::isfinite(initial_cost) && initial_cost > 0.0,
           "equirect BA starts from a finite cost");
    aetherscan::ba::OptimizerOptions options;
    options.fix_first_pose = true;
    const auto summary = aetherscan::ba::optimize_cpu(problem, options);
    ...
    for (std::size_t image = 1; image < poses.size(); ++image) {
        const aetherscan::ba::Pose& pose = problem.poses[image];
        const Mat3 rotation =
    ...
    aetherscan::ba::Problem coarse = problem;
    for (std::size_t image = 1; image < coarse.poses.size(); ++image) {
        aetherscan::ba::Pose& pose = coarse.poses[image];
    ...
    const double coarse_cost = aetherscan::ba::evaluate_cost(coarse, 2.0);
    aetherscan::ba::OptimizerOptions coarse_options = options;
    coarse_options.maximum_iterations = 60;
    const auto coarse_summary = aetherscan::ba::optimize_cpu(coarse, coarse_options);
    ...
    if (aetherscan::ba::CudaLinearizer::is_available()) {
        aetherscan::ba::LinearizationOutput cpu_output;
        aetherscan::ba::linearize_cpu(coarse, cpu_output);
        aetherscan::ba::CudaLinearizer device;
        device.upload(coarse);
        device.evaluate();
        aetherscan::ba::LinearizationOutput gpu_output;
...
void test_storage_and_export() {
    ...
    const auto encoded = aetherscan::sfm::encode_asfm(scene);
    const Scene decoded = aetherscan::sfm::decode_asfm(encoded);
    ...
    const std::filesystem::path directory =
        std::filesystem::temp_directory_path() / "aetherscan_equirect_export_test";
    std::error_code error;
    std::filesystem::remove_all(directory, error);
    aetherscan::sfm::save_colmap_text(scene, directory / "colmap", {}, false);
    ...
    bool rejected = false;
    try {
        aetherscan::sfm::export_openmvs_interface(scene, directory / "scene.mvs");
...
void test_camera_model_selection() {
    expect(aetherscan::sfm::is_equirectangular_image_size(2048, 1024),
           "2:1 image size is recognized");
    expect(!aetherscan::sfm::is_equirectangular_image_size(1920, 1080),
           "16:9 image size is not a panorama");
    expect(!aetherscan::sfm::is_equirectangular_image_size(0, 0),
    ...
    const auto selected = aetherscan::sfm::select_camera_model(
    ...
    const auto no_hint = aetherscan::sfm::select_camera_model(
    ...
    const auto explicit_request = aetherscan::sfm::select_camera_model(
    ...
    std::vector<aetherscan::sfm::CameraModelProbe> pinhole_probes;
    for (int pair = 0; pair < 6; ++pair) {
        const Pose3D second = make_pose(
    ...
    const auto rectilinear = aetherscan::sfm::select_camera_model(
