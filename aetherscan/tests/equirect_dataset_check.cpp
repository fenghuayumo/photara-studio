...
//
//   aetherscan_equirect_dataset_check <images> <out_colmap_dir>
...
int main(const int argc, char** argv) {
    ...
    config.frontend.camera_model =
        aetherscan::CameraModel::equirectangular;
    if (options.camera_model == "auto" || options.camera_model == "automatic")
        config.frontend.camera_model = aetherscan::CameraModel::automatic;
    else if (options.camera_model == "pinhole")
        config.frontend.camera_model = aetherscan::CameraModel::pinhole;
    else if (options.camera_model == "fisheye" ||
             options.camera_model == "opencv_fisheye")
        config.frontend.camera_model = aetherscan::CameraModel::opencv_fisheye;
    ...
    aetherscan::core::Logger::instance().set_levels(
        aetherscan::core::LogLevel::info, aetherscan::core::LogLevel::off);

    aetherscan::sfm::Scene scene;
    const auto started = std::chrono::steady_clock::now();
    const auto summary =
        aetherscan::sfm::reconstruct(scene, files, config);
    ...
    for (const auto& camera : scene.cameras)
        if (camera.model == aetherscan::CameraModel::equirectangular)
